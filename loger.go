...
func (p *ChLoger) Stop() {
	...
	fmt.Println(Getime(), "| ** END ** | Chan Logger |")
	return
}
...
func (p *ChLoger) SignalStoper(prepareExit <-chan bool) {
	...
	p.Stop()
	p.GoodBy <- true
	fmt.Println(Getime(), "| ** GoodBy <- END ** | Chan Logger |")
	return
}
...
func (p *ChLoger) exec(gopher int, elem [4]string) {
	...
	funcFlush = fmt.Sprintf("%s | FUNC:%s | UNIT: %s | TIP:%s |TEXT: 【%s】\n", Getime(), elem[0], elem[1], elem[3], elem[2])
	if elem[3] == "1" || elem[3] == "ERROR" { // Если ошибка
		errFlush := fmt.Sprintf("%s | UNIT: %s | FUNC: %s | TEXT: %s\n", Getime(), elem[1], elem[0], elem[2])
		if p.Options.Dir != "" {
			fileEtl := fmt.Sprintf("%s.txt", "Errors")
			fEtl, err := OpenWriteFile(filepath.Join(p.Options.Dir, fileEtl))
			FckText(fmt.Sprintf("Логгер | Открытие файла: %s", fileEtl), err)
	...
	if p.Options.Dir != "" {
		...
		fFunc, err := OpenWriteFile(filepath.Join(p.Options.Dir, fileFunc))
		FckText(fmt.Sprintf("Логгер | Открытие файла: %s", fileFunc), err)
